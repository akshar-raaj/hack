...
class SingleAnswerListThread(Thread):
    def __init__(self, url, number, *args, **kwargs):
        self.url = url
        self.number = number
        super(SingleAnswerListThread, self).__init__(*args, **kwargs)
...
while base_page_number < 6:
    url = base_url + str(base_page_number)
    t = SingleAnswerListThread(url, base_page_number)
    single_answer_list_threads.append(t)
    t.start()
    base_page_number = base_page_number + 1
for t in single_answer_list_threads:
    t.join()
print num_views